...
public class main_alg {
    ...
    // NOTE: Run the `analyze_effort.py` script to get the most accurate values from your data.
    ...
    // The INTERCEPT (in hours). This represents a fixed "startup cost" for any task,
    // regardless of its size (e.g., context switching, setup, understanding the task).
     ...
     * The model is based on a linear regression of thousands of real-world software tasks.
     *
     * @param estimatedTimeInMinutes The user's estimate of how long the task will take, in minutes.
     * @return A more realistic task duration in minutes, accounting for common estimation biases.
     */
    public static double getRealisticTimeInMinutes(double estimatedTimeInMinutes) {
        ...
        // As a sanity check, ensure the adjusted time is not less than the original estimate.
        return Math.max(estimatedTimeInMinutes, realisticTimeInMinutes);
    }

    public static void main(String[] args) {
        System.out.println("--- Effort Adjustment Model ---");
        System.out.println("This model adjusts a user's time estimate to a more realistic value based on real-world data.");
